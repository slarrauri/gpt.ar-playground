...
const editorSlice = createSlice({
  ...
  reducers: {
    ...
    cleanExampleList: (state) => {
      ...
      if (
        workspace.examples.length < 1 ||
        workspace.examples[workspace.examples.length - 1].text.length
      ) {
        workspace.examples.push({
          id: uniqid("input_"),
          text: "",
          output: undefined,
          isLoading: false,
    ...
    loadOutputForExample: (
      state,
      action: PayloadAction<LoadExampleOutputActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    deleteExample: (state, action: PayloadAction<string>) => {
      ...
      workspace.examples = workspace.examples.filter(
    ...
    normalizeConversations: (state) => {
      ...
      if (
        ...
        workspace.conversations[0].parts.length > 1
      ) {
        let startSequence = "\nAI:";
        let restartSequence = "\nUser: ";
    ...
    setConversationCompletionParams: (
      state,
    ...
    setConversationInitialPrompt: (
      state,
      action: PayloadAction<SetConversationInitialPromptActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    updateConversationLoadingStatus: (
      state,
      action: PayloadAction<UpdateConversationLoadingStatusActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    updateConversationInputValue: (
      state,
      action: PayloadAction<UpdateConversationInputValueActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    updateConversationStartSequence: (
      state,
      action: PayloadAction<UpdateConversationStartSequenceActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    updateConversationRestartSequence: (
      state,
      action: PayloadAction<UpdateConversationRestartSequenceActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    addMessageInConversation: (
      ...
      action: PayloadAction<
        | AddMessageToConversationFromUserActionPayload
        | AddMessageToConversationFromGptActionPayload
      >
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      ...
      workspace.conversations = workspace.conversations.map((conversation) => {
        ...
        lastPart.text = convertConversationPartToText(
          inputText,
          lastPart.source,
          conversation.startSequence,
          conversation.restartSequence
        );
    ...
    loadTemplate: (state, action: PayloadAction<LoadTemplateActionPayload>) => {
      ...
      if (action.payload.startSequence !== undefined) {
        workspace.conversations[0]!.startSequence =
          action.payload.startSequence;
      }
      if (action.payload.restartSequence !== undefined) {
        workspace.conversations[0]!.restartSequence =
          action.payload.restartSequence;
      }
      workspace.tabIndex = action.payload.tabIndex;
    },
    loadTemplateFromFileData: (
      state,
      action: PayloadAction<LoadTemplateFromFileDataActionPayload>
    ) => {
      let workspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
    ...
    deleteStopSymbol: (state, action: PayloadAction<string>) => {
      ...
      workspace.stopSymbols = workspace.stopSymbols.filter(
    ...
    updateWorkspaceId: (state, action: PayloadAction<string>) => {
      const newWorkspace = state.workspaces.find(
    ...
    createWorkspace: (state) => {
      let currentWorkspace = state.workspaces.find(
        (w) => w.id === state.currentWorkspaceId
      )!;
      const newWorkspace = {
        ...currentWorkspace,
...
const fetchForCurrentTab = (): AppThunk => (dispatch, getState) => {
  ...
  let workspace = state.editor.present.workspaces.find(
...
const fetchBasicOutputAsync = (): AppThunk => (dispatch, getState) => {
  ...
  GptAPI.generateCompletions(completionParams.prompt, completionParams)
    .then((response) => {
...
const fetchExamplesOutputsAsync = (): AppThunk => (dispatch, getState) => {
  ...
  const examples = workspace.examples.filter(
  ...
  GptAPI.generateCompletions(examplePrompts, completionParams)
    ...
    .then((response) => {
      response.choices.map((exampleResult: ChoiceResult, ind: number) => {
        ...
        dispatch(
          loadOutputForExample({ id: exampleId, output: exampleResult.text })
        );
        return undefined;
      });
    })
...
const fetchVariationsAsync = (): AppThunk => (dispatch, getState) => {
  ...
  GptAPI.generateCompletions(
    completionParams.prompt,
    completionParams,
    workspace.maxVariations
  )
    .then((response) => {
      console.log(response.data);
      return { ...response.data };
...
const sendMessageInConversationAsync =
  ...
  (dispatch, getState) => {
    ...
    if (conversation.parts.length === 1) {
      dispatch(
        setConversationInitialPrompt({
          ...
          initialPrompt: selectPrompt(state),
        })
      );
      dispatch(
        setConversationCompletionParams({
          conversationId: conversationId,
          parameters: selectCompletionParameters(state),
        })
      );
    }
    ...
    let updatedWorkspace = updatedState.editor.present.workspaces.find(
      (w) => w.id === state.editor.present.currentWorkspaceId
    )!;
    const updatedConversation = updatedWorkspace.conversations.find(
      (conversation) => conversation.id === conversationId
    );
    ...
    const completionParams = {
      ...
      ...updatedConversation.completionParams!,
    };
    const prompt =
      updatedConversation.initialPrompt +
      updatedConversation.parts.map((p) => p.text).join("");
    GptAPI.generateCompletions(prompt, completionParams)
      .then((response) => {
        console.log(response.data);
        return { ...response.data };
      })
...
const selectApiKeyDialogVisible = (state: RootState) =>
  state.editor.present.showApiKeyDialog;
const selectTemplateDialogVisible = (state: RootState) =>
  state.editor.present.showTemplateDialog;
const selectCurrentWorkspaceId = (state: RootState) =>
  state.editor.present.currentWorkspaceId;
const selectEditableWorkspaceName = (state: RootState) =>
  state.editor.present.editableWorkspaceName;
const selectCurrentWorkspaceName = (state: RootState) =>
  state.editor.present.workspaces.find(
    (w) => w.id === state.editor.present.currentWorkspaceId
  )!.name;
const selectWorkspacesList = (state: RootState) =>
  state.editor.present.workspaces.map((w) => ({ id: w.id, name: w.name }));
...
const selectCompletionParameters = (state: RootState) => {
  ...
  return {
    apiKey:
      state.editor.present.apiKey === undefined
        ? ""
        : state.editor.present.apiKey,
...
function convertConversationPartToText(
  text: string,
  source: ConversationPartSource,
  startSequence: string,
  restartSequence: string
...
export type {
  ...
  LoadTemplateActionPayload, // for templates library
};
