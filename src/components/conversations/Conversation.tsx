...
export default function Conversation(props: Props) {
  ...
  useEffect(() => {
    conversationBottom.current!.scrollTop =
      conversationBottom.current!.scrollHeight;
  });
            ...
            <Box ml={1} mt={1}>
              {hasStarted && (
                <>
                  <Typography
                    component={"span"}
                    className={styles.promptedText}
                  >
                    {conversation.initialPrompt}
                  </Typography>
                  {conversation.parts.map((part) => (
                    <>
                      {part.source === ConversationPartSource.gpt && (
                        <Typography
                          component={"span"}
                      ...
                      {part.source === ConversationPartSource.user && (
                        <Typography
                          component={"span"}
                          className={styles.promptedText}
                        >
            ...
            <AccordionDetails>
              <Typography>
                <Grid container spacing={1}>
                  <Grid item>
                    <TextField
                      value={conversation.restartSequence
                        .split("\n")
                        .join("\\n")}
                      onChange={(
                        ...
                        dispatch(
                          updateConversationRestartSequence({
                            conversationId: props.id,
                            restartSequence: event.currentTarget.value
                              .split("\\n")
                              .join("\n"),
                          })
                        );
                    ...
                    <TextField
                      ...
                      onChange={(
                        ...
                        dispatch(
                          updateConversationStartSequence({
                            conversationId: props.id,
                            startSequence: event.currentTarget.value
                              .split("\\n")
                              .join("\n"),
                          })
                        );
                ...
                <Box mt={1}>
                  {conversation.completionParams === undefined && (
                    <CompletionParameters
                      parameters={globalCompletionParameters}
                    />
                  )}
                  {conversation.completionParams !== undefined && (
                    <CompletionParameters
                      parameters={conversation.completionParams}
                    />
                  )}
                </Box>
              </Typography>
